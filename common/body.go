...
func Body(syntaxes ...WrappedSyntax) WrappedSyntax {
	...
	for i := len(syntaxes) - 1; i >= 0; i-- {
		var end int
		if list.SourceLocationTree() == nil {
			end = syntaxes[i].SourceLocation().Offset + syntaxes[i].SourceLocation().Length
		} else {
			end = list.SourceLocation().Offset + list.SourceLocation().Length
		}
		sourceLocationTree := SourceLocationTree{
			SourceLocation{
				...
				Length: end - syntaxes[i].SourceLocation().Offset,
			},
